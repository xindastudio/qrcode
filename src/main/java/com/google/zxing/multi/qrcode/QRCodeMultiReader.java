...
public final class QRCodeMultiReader extends QRCodeReader implements MultipleBarcodeReader {
  ...
  private static final class SAComparator implements Comparator<Result>, Serializable {
    ...
    public int compare(Result a, Result b) {
      int aNumber = Integer.valueOf((a.getResultMetadata().get(ResultMetadataType.STRUCTURED_APPEND_SEQUENCE)).toString());
      int bNumber = Integer.valueOf((b.getResultMetadata().get(ResultMetadataType.STRUCTURED_APPEND_SEQUENCE)).toString());
      if (aNumber < bNumber) {
        return -1;
      }
      if (aNumber > bNumber) {
